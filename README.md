...
ch585（以及其他ch57-ch59系列）出厂自带USB接口的ISP，速度和稳定性都比用调试口好很多，可在例程文件夹下运行`make -j99 flash`命令进行编译和自动下载。注意在运行前需要按住ch585开发板上的download按键并将usb接入开发板，随后松开download按键并在20秒内完成下载。
